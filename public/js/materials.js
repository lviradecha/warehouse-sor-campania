...
// ===================================
// 1. MATERIALS.JS - Export CSV
// ===================================

// Aggiungere il pulsante nel render():
/*
<div style="display: flex; gap: 10px;">
    <button class="btn btn-success" onclick="MaterialsPage.exportCSV()" title="Esporta tutti i materiali in CSV">
        📥 Esporta CSV
    </button>
    <button class="btn btn-primary" onclick="MaterialsPage.showAddModal()">
        ➕ Nuovo Materiale
...
// Aggiungere questa funzione in MaterialsPage:
exportCSV() {
    if (this.materials.length === 0) {
    ...
    const rows = this.materials.map(m => [
        ...
        m.stato || '',
        m.posizione || '',
        (m.note || '').replace(/"/g, '""'), // Escape virgolette
        m.data_acquisizione || '',
        m.valore || '',
        m.fornitore || '',
        m.numero_seriale || ''
